...
namespace Lumos
{
    namespace Maths
    {
        ...
        ////    glm::vec3 mina(FLT_MAX, FLT_MAX, FLT_MAX);
        ////    glm::vec3 maxa(-FLT_MAX, -FLT_MAX, -FLT_MAX);

        ////    for (uint32_t i = 0; i < numPoints; i++)
        ////    {
        ////        glm::vec3 transformed = transform * glm::vec4(points[i], 1.0f);

        ////        mina.x = glm::min(mina.x, transformed.x);
        ////        mina.y = glm::min(mina.y, transformed.y);
        ////        mina.z = glm::min(mina.z, transformed.z);

        ////        maxa.x = glm::max(maxa.x, transformed.x);
        ////        maxa.y = glm::max(maxa.y, transformed.y);
        ////        maxa.z = glm::max(maxa.z, transformed.z);
        ////    }
